...
def ler_arquivo(arquivo):
    if os.path.exists( arquivo ):
        with open( arquivo, "r" ) as arq1json:
            dicionario = json.load( arq1json )
    else:
        dicionario = {}
    return dicionario
...
def gravar_arquivo(dicionario, arquivo):
    with open( arquivo, "w" ) as arq1json:
        json.dump( dicionario, arq1json )
...
#       resp = input( "Digite <S> para continuar ou ." ).upper()
